...
class ControllerExtensionPaymentZalopayAtm extends Controller {
	...
	public function index() {
        ...
        try
        {
            ...
            $this->model_extension_payment_zalopay_atm->addOrder($order, $order_data['app_trans_id']);
        ...
        {
            return $this->load->view($this->config->get('config_template').'/template/extension/payment/zalopay_atm', $zalopay_order);
        } 
        else 
        {
            return $this->load->view('extension/payment/zalopay_atm', $zalopay_order);
        }
    ...
    public function callback() {
        ...
        try{
            ...
            if($response["return_code"]){
                ...
                $orderId = $this->model_extension_payment_zalopay_atm->getZaloPayOrderId($_data['app_trans_id']);
                if( $orderId > 0 ){
    ...
    public function redirect() {
        ...
        try{
            ...
            if(isset($requestData["status"]) && $requestData["status"] == 1){
                $orderId = $this->model_extension_payment_zalopay_atm->getZaloPayOrderId($requestData['apptransid']);
                // Checksum
